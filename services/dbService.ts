// PROTÓTIPO LEGADO — ARMAZENAMENTO LOCAL (IndexedDB) SOMENTE.
//
// Este serviço pertence ao modo de demonstração local e NÃO acessa o banco
// institucional. Toda sincronização automática com o Supabase foi removida:
// não há mais upload de JSONB nem `migrateLocalToRemote`. A importação de
// dados legados para o núcleo normalizado é feita exclusivamente por um
// importador administrativo auditável no backend (ver database/importer),
// nunca pelo navegador.
//
// Dados aqui são visíveis apenas neste navegador e não devem ser usados para
...
class DBHelper {
  ...
  getConnectionInfo(): LegacyConnectionInfo {
    return {
      ...
      isRemoteConfigured: false,
      message:
        'Modo legado local: os dados ficam apenas neste navegador (IndexedDB) e não são centralizados.',
    };
  }
